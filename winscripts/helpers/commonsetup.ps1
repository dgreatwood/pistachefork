  ...
  ((where.exe pkg-config) -like "*Strawberry*")) {
      ...
      { # winget exists on newer versions of Windows 10 and on
        ...
        # 2022.
          ...
          # winget
          $tmp = winget install bloodrock.pkg-config-lite
...
# Note: Using Get-Command like this:
#     if ((! (Get-Command python3 -errorAction SilentlyContinue)) ...
# doesn't work for python/python3 because Windows treats python as a
# special command that always exists; if python is invoked when it is
# not installed, Windows outputs a helpful message on how to install
# it via app store. Anyway, it means we have to use a different method
# to determine if python is already installed
