...
TEST(https_client_test, one_client_with_google_request)
{
    ...
    const std::string server_address(
        "https://www.google.com/search?q=pistache+C%2B%2B+HTTP+REST+API");
    ...
    auto response = rb.header<Http::Header::Connection>(Http::ConnectionControl::KeepAlive)
                        .send();
    bool done                       = false;
    bool found_pistache_github_page = false;

    response.then(
        [&done, &found_pistache_github_page](Http::Response rsp) {
            if (rsp.code() == Http::Code::Ok)
            {
                done = true;
                const std::string bdy(rsp.body());
                found_pistache_github_page = (bdy.find("github.com/pistacheio/pistache") != std::string::npos);
            }
        },
    ...
    ASSERT_TRUE(done);
    ASSERT_TRUE(found_pistache_github_page);
}
...
TEST(https_client_test, multiple_clients_with_multiple_search_requests)
{
    ...
    const std::string server_address_start[2] {
    ...
    for (unsigned int j = 0; j < CLIENT_SIZE; ++j)
    {
        ...
        {
            const std::string server_address(
                server_address_start[(i + j) % 2] + query[i]);
                 ...
                 resp_substr](Http::Response rsp) {
                    ...
                    if (rsp.code() == Http::Code::Ok)
                    {
                        ...
                        if (it != bdy.end())
                            ...
                            PS_LOG_DEBUG_ARGS(
                                "For i=%d, j=%d, %s not found in resp body %s",
                                i, j,
    ...
    // "CLIENT_SIZE * RESPONSE_SIZE", because both Bing and Google appear to
    // intermittently return flaky results sometimes - pages that offer to
    // redirect us to their AI result and so on and which don't actually
    // include the answer as far as I could see.
    ASSERT_GE(response_correct_counter, ((CLIENT_SIZE * RESPONSE_SIZE) * 2) / 3);
}
...
TEST(https_client_test, one_client_multiple_requests_force_https_verification)
{
